...
msg_handler = logging.FileHandler("messages.log", encoding="utf-8")
...
def log_message(username: str, user_id: str, direction: str, text: str) -> None:
    ...
    Format: time date;Username;user ID;[snd/rsv];msg text
    / Логирует обмен сообщениями с пользователем.
    direction: 'snd' (отправлено) или 'rsv' (получено)
    Формат: time date;Имя пользователя;user ID;[snd/rsv];msg text
    """
    now = datetime.now()
